...
public class parta {
	...
	public static void partA (String start, String end, Graph graph, JSONObject jsonObj) {
		...
		// Unvisited Set: Stores visited nodes that has not been expanded -> list of nodes to visit
		...
		while (unvisited.size() > 0) {
			...
			for (Object node : connectedNodes) {
				...
				// Node is visited before and is in the visited/unvisited sets
					...
					// If cumulated distance is smaller than cumulated distance of the neighbour node to current node
					// we assign new cumulated distance
					if (pathCost.get(nodeStr) > cumulatedDist) {
						
						// Assigning updated cumulative energy and distance costs for respective neighbour node
						pathCost.put(nodeStr, cumulatedDist);
						energyCost.put(nodeStr, cumulatedEnergy);
						prevNode.put(nodeStr, curNode);
