...
public class partc {
	...
	public static void partC(String start, String end, Graph graph, JSONObject jsonObj) {
		...
		while (unvisited.size() > 0) {
			...
			// If cost function of node is lesser than the current node, we assign node as
			// the new current node
			// cost function = cumulative distance between node and start node + straight
			// line distance from node to target node
			for (String node : unvisited) {
				if (curNode.equals("") || pathCost.get(node) + graph.getHeuristic().get(node) < pathCost.get(curNode)
						+ graph.getHeuristic().get(curNode)) {
					curNode = node;
				}
			}
			...
			// For each node in the neighbour node array, we check whether it is completely
			// unvisited
